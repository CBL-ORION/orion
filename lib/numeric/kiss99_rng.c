...
#define kiss99_rng_VNI(state) ((long) kiss99_rng_KISS(state))*4.656613e-10
...
int main(void) {
	int i; uint32_t k;
	kiss99_rng_state s;
	kiss99_rng_state* state = &s;
	kiss99_rng_set_table(state, 12345,65435,34221,12345,9983651,95746118);
	for(i=1;i<1000001;i++){k=kiss99_rng_LFIB4(state);} printf("%u\n", k-1064612766U);
	for(i=1;i<1000001;i++){k=kiss99_rng_SWB(state) ;} printf("%u\n", k- 627749721U);
	for(i=1;i<1000001;i++){k=kiss99_rng_KISS(state) ;} printf("%u\n", k-1372460312U);
	for(i=1;i<1000001;i++){k=kiss99_rng_CONG(state) ;} printf("%u\n", k-1529210297U);
	for(i=1;i<1000001;i++){k=kiss99_rng_SHR3(state) ;} printf("%u\n", k-2642725982U);
	for(i=1;i<1000001;i++){k=kiss99_rng_MWC(state) ;} printf("%u\n", k- 904977562U);
	for(i=1;i<1000001;i++){k=kiss99_rng_FIB(state) ;} printf("%u\n", k-3519793928U);
}
/*-----------------------------------------------------
